...
test2 = test[,-c(1:5,7:10,12,14,53)] 
  ...
  x = train2[,-5]
  x = data.matrix(x) 
cv = cv.glmnet(x, train2[, 5], alpha = 0.5) 
```
...
#mean((cv.pred-test2$Second.Bid)^2) 
...
Some errors 
```{r}
mean((cv.model-train2$Second.Bid)^2) # Regularized error, train 
mean((cv.predict-testn2$Second.Bid)^2) # Regularized error, test
mean((old.model - train2$Second.Bid)^2) #OLS error 
mean((ols.predict-test2$Second.Bid)^2) 
#mean((reg.predict.ols - train2$Second.Bid)^2) #OLS error 
```

More errors. What about the model training errors? 
```{r}
(mean((cv.pred-train2$Second.Bid)^2)) / (mean((reg.predict.ols - train2$Second.Bid)^2))
```


What about the model test errors? 
```{r}
...
reg.model.full = glmnet(data.matrix(full2[,-5]), full2[, 5], alpha=0.5)#,lambda=0) 
reg.predict.full = predict(reg.model.full, type="coefficients", s = cv$lambda.1se) 
reg.predict.full 
```
