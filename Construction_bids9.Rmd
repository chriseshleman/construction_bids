...
![Project map](Estimator_Data.png) 
...
Running a regularization process can weed out weaker variables and identify one or two key items to help prediction. The process would shrink (toward zero) the estimates for covariates that threaten to introduce more uncertainty than predictive power. The need for manageability meant adopting some basic assumptions from the modeling software (10 folds, standardized coefficients, use of mean squared error as an evaluation metric, et cetera). 
