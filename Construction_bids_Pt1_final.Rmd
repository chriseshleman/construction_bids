...
#1. Summary
The Port Authority estimates project costs internally prior to bidding. Its accuracy varies from project to project. The statistical analysis described here indicates two factors — project size and bidding process — explain a noteworthy degree of accuracy. The agency may be able to more accurately predict costs by adjusting initial estimates to account for those two variables. This could help inform capital capacity estimates and minimize the need for project change orders.
...
Discussion is broken down into an overview of data sets, exploratory analysis, methodology, and summary with next steps. 
...
#2. Motivation
The Engineering Department's internal cost estimates are arguably strong when evaluated against the second-lowest bid. Internal agency policy requires it to accept the lowest bid, which at times means approving a bid-driven project cost reasonably believed to be too low. This has implications for capital planning and budgeting, and can require the use of project-level budget offsets as actualized project costs come in higher than contractor expectations. Data on actual costs for projects — many of which span multiple years in construction — is not available for much of the data set. The use, therefore, of the second-lowest bid instead of lowest bid provides the most defensible tool in evaluating the expected actual cost of projects. 

The agency's cost estimating team is very accurate, particularly when aggregating the over- and under-estimates - effectively canceling much of the error away. On an absolute basis^[Mean absolute error, MAE.], however, the gap between internal estimate and second-lowest bid averages $2.7 million, or 18 percent of the average project size. Reducing this gap would provide for stronger confidence in long-range capital capacity estimates and could reduce the need for project-level change orders. 
...
mae(bids$Second.Bid,bids$Engr.Est) 
mae(bids$Second.Bid,bids$Engr.Est)/mean(bids$Second.Bid)
```
...
bids$Format2 = relevel(bids$Format2, ref = "Public")
```
...
#, labels = c("$0-1.45M","$1.45-2.47M", "$2.47-$4.65M","$4.65-13.32M","$13.32M-4.8B")
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
ggplot(bids, aes(x=log(Engr.Est), y=log(Second.Bid), color=Typeology)) +  geom_point(size=2,shape=1) +
  geom_smooth(method=lm, se=FALSE, aes(group=1)) + 
  annotate(geom="text", x=14, y=19.5, label="Correlation", #"Scatter plot"
              color="red") +
  annotate(geom="text", x=14, y=19, label=a, #"Scatter plot"
              color="red") 
```
...
##5a. Base model (manual selection and linear regression)
...
Choose a handful of potential predictors and build a linear model. Go with the original estimate (always included) and (1) regional, construction-specific employment, (2) bidding process (public or other), (3) typeology (infrastructure, building, or paving), (4) construction permits (lagged by one quarter), (5) construction costs (indexed), and (6) project size, with project dollars (from the second bid) categorized in quintiles.
...
##5b. Shrinkage and automated variable selection.
...
This analysis uses a blended regularization penalty that falls between ridge and lasso (the "alpha" is set at 0.3):
...
names(train) 
blend = 0.15 # pick penalization process (between 0 and 1)
train2 = train[,-c(1:5,7:10,12,14,34,53,57:58)] 
  train2 = na.omit(train2) 
test2 = test[,-c(1:5,7:10,12,14,34,53,57:58)] 
  test2 = na.omit(test2) 
  x = train2[,-5]
names(train2) 
    write.csv(train2, "./Train_for_training_Pt1.csv") 
    write.csv(x, "./X_for_training_Pt1.csv")
...
full2 = rbind(train2,test2) 
  ...
  x = data.matrix(x) 
set.seed(987) 
full.model = glmnet(data.matrix(full2[,-5]), full2[, 5], alpha=.25, lambda=cv$lambda.1se) 
full.predict = predict(full.model, type="coefficients", s = cv$lambda.1se, keep=TRUE) 
```
...
Next steps include looking for best practices regarding post-estimation adjustments for project cost by size and/or bidding process. We will also conduct targeted explorations of those two subpopulations. The Engineering Department should begin tracking project accuracy using mean absolute error as a second metric. 
...
final_short1 = lm(bids$Second.Bid ~ bids$Engr.Est + bids$quantile_5) 
final_short2 = lm(bids$Second.Bid ~ bids$Engr.Est + bids$Format2) 
```

```{r, echo=FALSE, include=TRUE}
write.csv( tidy(final) , "./coefs.csv" )
write.csv( glance(final) , "./an.csv" )
write.csv(final$fitted.values,"./Predictions 20191120.csv") 
write.csv(bids,"./Bids_data_set.csv") 
write.csv(tidy(final_short1), "coefs_short1.csv") 
write.csv(tidy(final_short2), "coefs_short2.csv") 

final.data = bids[,-c(1:5,7:10,12,14,16,34,53,57:59)] 
  final.data = na.omit(final.data) 
  final.x = final.data[,-5]
  final.x = data.matrix(final.data) 

set.seed(882) 
final.cv = cv.glmnet(final.x, bids[, 16], alpha = .15, nfolds = 5) 
set.seed(992) 
final.cv.predict = predict(final.cv, s=cv$lambda.1se ,newx=final.x)
coef(final.cv) 
write.csv(tidy(final),"./Coefficients_CV.csv") 
write.csv(final.cv.predict, "./Predictions_CV.csv") 
```

This model provides an incremental improvement in MAE, both raw and as a share of project size: 
```{r, echo=TRUE, include=TRUE} 
mae(bids$Second.Bid,final$fitted.values) 
mae(bids$Second.Bid,final$fitted.values)/mean(final$fitted.values)
```

\pagebreak

#Appendix:exploratory work, model outputs.
...
ggplot(bids, aes(x = as.factor(quantile), y = accuracy)) + 
    geom_hline(colour="dark gray", yintercept=1) +
    geom_jitter(width=0.2) +
    geom_crossbar(data=quintile.summary, aes(ymin = accuracy, ymax = accuracy),
                  size=1,col="red", width = .5) + 
      ylab("Second Bid ÷ Estimate") + 
    theme(axis.title.y=element_text(size=10)) + 
      annotate(geom="text", x=2, y=3.5, label="p-value", 
...
```{r, echo=FALSE, include=FALSE}
write.csv(bids,"./Bids from Pt1.csv") 
```
