...
If there are outliers in there - projects that, for an unexplainable reason, was way off, consider removing them. 
...
Some of the remaining variation can be explained with some guesswork. Calculate summary stats to help visualize some obvious potential predictors: 
...
Location likely has some predictive power that engineers may not be able to capture or fully predict. Basically, projects that span the Hudson River wind up costing more, on average, than ones plunked squarely in either New York or New Jersey. A bivariate look is an easy clue as to the potential role in a multivariate controlled relationship. Plots for this and subsequent exploratory work is at the end of the document; all consider accuracy, defined here as a ratio of the second-highest bid (a rule-of-thumb target) over the internal agency estimate.

- Location may matter: projects that span the Hudson River seem to come in, on average, higher than expected. 
...
The agency can predict paving projects fairly well.
...
Four obvious outliers, two apiece for PATH and TB&T, push the average accuracy metric up for each, otherwise there's little apparent difference across departments. 
...
These are the most surprising results. I would have expected the largest or smallest projects to be the tough to predict - particularly smaller projects. But it's the projects ranging from the 40th to 70th percentiles that seem to be the most challenging. 
...
Note: ensure the "accuracy" variable calculated earlier is dropped before modeling or introducesome dual (reverse) causality, which could confuse models.
